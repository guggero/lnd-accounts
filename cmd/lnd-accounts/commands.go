...
import (
	...
	macaroonsT "github.com/lightningnetwork/lnd/macaroons"
...
var accountMacaroonCommand = cli.Command{
	...
	Action: accountMacaroon,
}

func accountMacaroon(ctx *cli.Context) error {
	client, _, cleanUp := getClient(ctx)
	defer cleanUp()
	...
	copy(id[:], accountID)
	constrainedMac, err := macaroonsT.AddConstraints(
...
func createAccount(ctx *cli.Context) error {
	...
	_, acctClient, cleanUp := getClient(ctx)
	defer cleanUp()
	...
	req := &acctrpc.CreateAccountRequest{
		AccountBalance: initialBalance,
		ExpirationDate: expirationDate,
	}
	resp, err := acctClient.CreateAccount(ctxb, req)
	if err != nil {
		return err
	}
...
func listAccounts(ctx *cli.Context) error {
	...
	req := &acctrpc.ListAccountsRequest{}
	resp, err := acctClient.ListAccounts(ctxb, req)
	if err != nil {
		return err
	}
...
func removeAccount(ctx *cli.Context) error {
	...
	req := &acctrpc.RemoveAccountRequest{
		Id: accountID,
	}
	_, err := acctClient.RemoveAccount(ctxb, req)
	return err
}
